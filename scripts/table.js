...
function updateTable(data, year, dtList, pvList, selector) {
    ...
    const filteredData = data.filter(d => d.year == year && (dtListStr.includes(String(d.area_code)) || pvList.includes(d.th_province))); 
    const sortData = filteredData.sort((a,b)=> (parseInt(a.area_code) - parseInt(b.area_code) || 
                                                a.th_province.localeCompare(b.th_province) || parseInt(a.age_type) - parseInt(b.age_type) ||
                                                parseInt(a.sex) - parseInt(b.sex)));
    console.log(dtList);
    const keyValue = ['area_code', 'th_province', 'age_type', 'sex', 'LE', 'HALE', 'LE_HALE'];
        ...
        keyValue.forEach(key => {
            ...
            if (key == 'area_code') {
                ...
                if (item[key] == checkSex) {
                    ...
                    cell.textContent = item[key] == 1 ? 'เพศชาย' : item[key] === 2 ? "เพศหญิง": 'รวมเพศ';
                    row.appendChild(cell);
                    checkSex = item[key]
                }
            } else {
