...
function updateBarChart(data, year, dt, type, sex, colorLeft, colorRight, selector) {
    ...
    const filteredData = data.filter(d => d.year == year && d.age_type == type && d.sex == sex && d.area_code == dt); 
...
function updateHBarChart(data, year, dt, type, sex, colorLE, colorHALE, selector) {
    const filteredData = data.filter(d => d.year == year && d.area_code == dt && d.age_type == type && d.sex == sex); 
...
function updateHBarChartPV(data, year, dt, type, sex, colorLE, colorHALE, selector) {
    ...
    margin: {l: 70, r: 0, t: space, b: space},
    bargap: 0,
    font: {
        family: 'IBM Plex Sans Thai', // Assuming you have this font loaded in your HTML
    },
...
updateBarChart(dtData, filters.year, filters.dt, 0, 1, m_cl_dark, m_cl_light, "dt-male-at-birth");
updateBarChart(dtData, filters.year, filters.dt, 60, 1, m_cl_dark, m_cl_light, "dt-male-at-60");
updateBarChart(dtData, filters.year, filters.dt, 0, 2, fm_cl_dark, fm_cl_light, "dt-female-at-birth");
updateBarChart(dtData, filters.year, filters.dt, 60, 2, fm_cl_dark, fm_cl_light, "dt-female-at-60");
updateBarChart(dtData, filters.year, filters.dt, 0, 3, b_cl_dark, b_cl_light, "dt-bothsex-at-birth");
updateBarChart(dtData, filters.year, filters.dt, 60, 3, b_cl_dark, b_cl_light, "dt-bothsex-at-60");
updateCardValue(dtData, filters.year, filters.dt, 0, 1, 'LE', m_cl_dark, "dt-male-at-birth-le");
updateCardValue(dtData, filters.year, filters.dt, 0, 1, 'HALE', m_cl_light, "dt-male-at-birth-hale");
updateCardValue(dtData, filters.year, filters.dt, 60, 1, 'LE', m_cl_dark, "dt-male-at-60-le");
updateCardValue(dtData, filters.year, filters.dt, 60, 1, 'HALE', m_cl_light, "dt-male-at-60-hale");
updateCardValue(dtData, filters.year, filters.dt, 0, 2, 'LE', fm_cl_dark, "dt-female-at-birth-le");
updateCardValue(dtData, filters.year, filters.dt, 0, 2, 'HALE', fm_cl_light, "dt-female-at-birth-hale");
updateCardValue(dtData, filters.year, filters.dt, 60, 2, 'LE', fm_cl_dark, "dt-female-at-60-le");
updateCardValue(dtData, filters.year, filters.dt, 60, 2, 'HALE', fm_cl_light, "dt-female-at-60-hale");
updateCardValue(dtData, filters.year, filters.dt, 0, 3, 'LE', b_cl_dark, "dt-bothsex-at-birth-le");
updateCardValue(dtData, filters.year, filters.dt, 0, 3, 'HALE', b_cl_light, "dt-bothsex-at-birth-hale");
updateCardValue(dtData, filters.year, filters.dt, 60, 3, 'LE', b_cl_dark, "dt-bothsex-at-60-le");
updateCardValue(dtData, filters.year, filters.dt, 60, 3, 'HALE', b_cl_light, "dt-bothsex-at-60-hale");
updateHBarChart(dtData, filters.year, filters.dt, filters.ageType, 1, m_cl_dark, m_cl_light, 'pv-male-dt');
updateHBarChart(dtData, filters.year, filters.dt, filters.ageType, 2, fm_cl_dark, fm_cl_light, 'pv-female-dt');
updateHBarChart(dtData, filters.year, filters.dt, filters.ageType, 3, b_cl_dark, b_cl_light, 'pv-bothsex-dt');
updateHBarChartPV(pvData, filters.year, filters.dt, filters.ageType, 1, m_cl_dark, m_cl_light, 'pv-male-pv');
updateHBarChartPV(pvData, filters.year, filters.dt, filters.ageType, 2, fm_cl_dark, fm_cl_light, 'pv-female-pv');
updateHBarChartPV(pvData, filters.year, filters.dt, filters.ageType, 3, b_cl_dark, b_cl_light, 'pv-bothsex-pv');
...
document.getElementById('download-csv').addEventListener('click', function(e) {
    ...
    const dtData = JSON.parse(sessionStorage.getItem('dtData'));
    const pvData = JSON.parse(sessionStorage.getItem('pvData'));
    const filteredDtData = dtData.filter(d => d.year == filters.year && d.area_code == filters.dt);
    const filteredPvData = pvData.filter(d => d.year == filters.year && d.age_type == filters.ageType && d.area_code == filters.dt); 
    const filteredPvData2 = filteredPvData.map(x => ({ year: x.year, area_code: x.area_code, type: x.age_type, sex: x.sex, 
