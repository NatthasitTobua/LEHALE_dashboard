...
function createLineChart(data, type, metric, selector) {
    ...
    uniqueGenders.forEach(gender => {
        ...
        traceData.push({
            ...
            line: { shape: 'spline' },
            name: sexLabel,
            customdata: filteredByGender.map(d => [d.sex == 1 ? "ชาย" : d.sex  == 2 ? "หญิง" : "รวมเพศ"]),
...
document.getElementById('download-csv').addEventListener('click', function(e) {
  ...
  const filteredData = data.filter(d => d.age_type == filters.ageType);
  const columnMapping = {
    'year': 'ปี พ.ศ.',
    'sex': 'เพศ',
    'age_type': 'การคำนวณ'
