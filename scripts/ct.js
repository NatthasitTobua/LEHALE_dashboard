window.addEventListener("DOMContentLoaded", async (event) => {
  ...
  const pv_map = "https://raw.githubusercontent.com/Bodthailand/LEHALE/main/thailand_map.geojson";
  fetch(pv_map).then(response => response.json()).then(mapData => {
    sessionStorage.setItem('mapData', JSON.stringify(mapData));
  });

  // Connect google sheet by id
  const SheetID = "1tsZsS2JrbqzDVNd8WLY1Em_wjEIGrk3fMiHdfv1IS2o";
  const acRes = getSheetData({
    sheetID: SheetID,
    sheetName: "area_code", 
    query: "SELECT *"
  });

  const ctRes = getSheetData({
    sheetID: SheetID,
    sheetName: "country", 
    query: "SELECT *" 
  });

  const dtRes = getSheetData({
    sheetID: SheetID,
    sheetName: "district", 
    query: "SELECT *" 
  });

  const pvRes = getSheetData({
    sheetID: SheetID,
    sheetName: "province", 
    query: "SELECT *" 
  });

  const llRes = getSheetData({
    sheetID: SheetID,
    sheetName: "province_lat_lon", 
    query: "SELECT *" 
  });
  ...
  try {
    ...
    const [acData, ctData, dtData, pvData, llData] = await Promise.all([acRes, ctRes, dtRes, pvRes, llRes]);

    const map1 = new Map(acData.map(d => [d.post_code, d]));
    const map2 = new Map(llData.map(d => [d.post_code, d]));

    // Join province and lat_lon data
  ...
  function createBarChart(data, year, type, sex, colorLeft, colorRight, selector) {
    ...
    const filteredData = data.filter(d => d.year == year && d.age_type == type && d.sex == sex); 
    const trace = {
      x: ['<b>LE</b>', '<b>HALE</b>'],
  ...
  createBarChart(data, filters.year, 0, 3, b_cl_dark, b_cl_light,'both-at-birth');
  createBarChart(data, filters.year, 60, 3, b_cl_dark, b_cl_light,'both-at-60');
  createBarChart(data, filters.year, 0, 1, m_cl_dark, m_cl_light,'male-at-birth');
  createBarChart(data, filters.year, 60, 1, m_cl_dark, m_cl_light,'male-at-60');
  createBarChart(data, filters.year, 0, 2, fm_cl_dark, fm_cl_light,'female-at-birth');
  createBarChart(data, filters.year, 60, 2, fm_cl_dark, fm_cl_light,'female-at-60');
  updateCardValue(data, filters.year, 0, 3, 'LE', b_cl_dark, 'both-at-birth-le');
  updateCardValue(data, filters.year, 0, 3, 'HALE', b_cl_dark, 'both-at-birth-hale');
  updateCardValue(data, filters.year, 60, 3, 'LE', b_cl_dark, 'both-at-60-le');
  updateCardValue(data, filters.year, 60, 3, 'HALE', b_cl_dark, 'both-at-60-hale');
  updateCardValue(data, filters.year, 0, 1, 'LE', m_cl_dark, 'male-at-birth-le');
  updateCardValue(data, filters.year, 0, 1, 'HALE', m_cl_dark, 'male-at-birth-hale');
  updateCardValue(data, filters.year, 60, 1, 'LE', m_cl_dark, 'male-at-60-le');
  updateCardValue(data, filters.year, 60, 1, 'HALE', m_cl_dark, 'male-at-60-hale');
  updateCardValue(data, filters.year, 0, 2, 'LE', fm_cl_dark, 'female-at-birth-le');
  updateCardValue(data, filters.year, 0, 2, 'HALE', fm_cl_dark, 'female-at-birth-hale');
  updateCardValue(data, filters.year, 60, 2, 'LE', fm_cl_dark, 'female-at-60-le');
  updateCardValue(data, filters.year, 60, 2, 'HALE', fm_cl_dark, 'female-at-60-hale');
...
// const ac_path = "https://raw.githubusercontent.com/Bodthailand/LEHALE/main/area%20code%20(1).csv";
// const ll_path = "https://raw.githubusercontent.com/Bodthailand/LEHALE/main/province_lat_lon.csv";
// const pv_map = "https://raw.githubusercontent.com/Bodthailand/LEHALE/main/thailand_map.geojson";
