...
function createDropdownType(data, default_value, selector) {
    const uniqueTypes = [...new Set(data.map(d => d.age_type))];
    const dropdown = document.getElementById(selector);
...
function createDropdownSex(default_value, selector) {
    const uniqueSex = [1, 2, 3];
    const dropdown = document.getElementById(selector);
    uniqueSex.forEach(sex => {
    const option = document.createElement('option');
    option.value = sex;
    option.text = sex === 1 ? "เพศชาย" : sex === 2 ? "เพศหญิง" : "รวมเพศ";
    dropdown.appendChild(option);
    });
    $(dropdown).select2({
...
function createLineChart(data, dt, type, sex, metric, selector) {
    ...
    const filteredData = data.filter(d => d.area_code == dt && d.age_type == type && d.sex == sex);
    ...
    uniquePVs.forEach(pv => {
        ...
        traceData.push({
            ...
            line: { shape: 'spline' },
            name: pv,
            customdata: filteredByPV.map(d => [d.th_province, d.sex == 1 ? "ชาย" : d.sex  == 2 ? "หญิง" : "รวมเพศ"]),
    ...
    const sexThaiAdj = sex == 1 ? ' (เพศชาย)' : sex == 2 ? ' (เพศหญิง)' : ' (รวมเพศ)';
    const layout = {
        title: '<b>แนวโน้มอายุคาดเฉลี่ย'+metricThaiAdj+typeThaiAdj+'<br>('+metric+' '+typeEngAdj+') เขตสุขภาพที่ '+dt+sexThaiAdj+'</b>',
        yaxis: {
          range: yrange,
        },
        xaxis: {
...
function downloadImage(format, id) {    
    ...
    html2canvas(element, {
        ...
        link.href = dataUrl;
        link.download = 'แนวโน้มอายุคาดเฉลี่ยระดับเขตสุขภาพ'+'.'+format;
        link.click();
...
document.getElementById('download-csv').addEventListener('click', function(e) {
    ...
    const pvData = JSON.parse(sessionStorage.getItem('pvData'));
    const filteredDtData = dtData.filter(d => d.area_code == filters.dt && d.age_type == filters.ageType && d.sex == filters.sex);
    const filteredPvData = pvData.filter(d => d.area_code == filters.dt && d.age_type == filters.ageType && d.sex == filters.sex); 
