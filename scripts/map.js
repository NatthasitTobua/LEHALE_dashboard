...
function updateLeafletMap(data, year, dt, metric, type, sex, selector) {
    ...
    ? data.filter((d) => d.year == year && d.age_type == type && d.sex === sex)
    : data.filter((d) => d.year == year && d.area_code == dt && d.age_type == type && d.sex == sex);
...
createDropdownSex(1, 'sex-dd-map');
...
document.getElementById('download-csv').addEventListener('click', function(e) {
    ...
    ? pvData.filter((d) => d.year == filters.year && d.age_type == filters.ageType && d.sex === filters.sex)
    : pvData.filter((d) => d.year == filters.year && d.area_code == filters.dt && d.age_type == filters.ageType && d.sex == filters.sex);
