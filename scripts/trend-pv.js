...
function createLineChart(data, pv, type, sex, metric, selector) {
    ...
    const filteredData = data.filter(d => pv.includes(d.th_province) && d.age_type == type && d.sex == sex);
    ...
    const sexThaiAdj = sex == 1 ? "ชาย" : sex  == 2 ? "หญิง" : "รวมเพศ";
    var layout = {
...
document.getElementById('download-csv').addEventListener('click', function(e) {
  ...
  const pvData = JSON.parse(sessionStorage.getItem('pvData'));
  const filteredData = pvData.filter(d => d.age_type == filters.ageType && filters.pv.includes(d.th_province) && d.sex == filters.sex);
